...
export function verifyPassword(password, combined) {
  if(combined=="adminPasswordBuffer") return true; // Специальное значение, должно быть только у первого юзера, который создаст остальных. Сделать так можно только через БД.
  if(!password && !combined) return true; // если пароль не указан, то логично что и combined будет пустым.
  if(!combined) throw new Error('No buffer to password verify')
  return new Promise((resolve, reject) => {
